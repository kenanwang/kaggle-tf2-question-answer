...
class Span(object):
  """A class for handling token and byte spans.
    ...
    1) if start_token != -1 and end_token != -1 then the span is define
       by token offsets
    2) else, this is a null span.
  ...
  def __init__(self, start_token_idx, end_token_idx):
  ...
  def is_null_span(self):
    ...
    if (self.start_token_idx < 0 and self.end_token_idx < 0):
      return True
    return False
...
def nonnull_span_equal(span_a, span_b):
  ...
  if (span_b.start_token_idx >= 0 and span_b.end_token_idx >= 0):

    if (span_a.end_token_idx == span_b.end_token_idx):
      return True
...
def read_prediction_json(predictions_path):
  ...
  for single_prediction in predictions['predictions']:
    ...
    if 'long_answer' in single_prediction:
      long_span = Span(single_prediction['long_answer']['start_token'],
                       single_prediction['long_answer']['end_token'])
    else:
      long_span = Span(-1, -1)  # Span is null if not presented.

    short_span_list = []
    if 'short_answers' in single_prediction:
      for short_item in single_prediction['short_answers']:
        short_span_list.append(
            Span(short_item['start_token'], short_item['end_token']))
  ...
  with GzipFile(fileobj=gzipped_input_file) as input_file:
    for line in input_file:
      ...
      for annotation in json_example['annotations']:
        ...
        long_span = Span(long_span_rec['start_token'],
                         long_span_rec['end_token'])

        short_span_list = []
        for short_span_rec in annotation['short_answers']:
          short_span = Span(short_span_rec['start_token'],
